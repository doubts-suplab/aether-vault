...
**Current status:** Phase 1 — Ingestion & Retrieval Engine ✅ complete. Phase 2 — Knowledge Graph Extraction 🔄 core complete: automatic entity + co-occurrence relation extraction during ingestion via a pluggable `EntityExtractor` (dependency-free heuristic default) and `GraphExtractionPort`, wired as best-effort augmentation to the ingest pipeline; plus **entity-aware RAG** — `RagPipelinePort.retrieveWithGraph` recognises entities in the query, resolves them against the collection graph, expands to their neighbours, and returns a bounded `GraphContext` (matched + related entities + a prompt-ready summary) alongside the text `RagContext`, opt-in via `includeGraph` on `POST /rag/query` and best-effort (plain RAG is unaffected if the graph is absent or a lookup fails); plus a **selectable NER strategy** — a config-gated model-based `OllamaEntityExtractor` (`aether.vault.graph.extractor=llm`, JSON-mode prompt, **falls back to the heuristic** on any failure) — and **entity resolution/de-dup** (a `NormalizingEntityResolver` behind an `EntityResolver` port, wrapped via `ResolvingEntityExtractor` so surface-form variants collapse to one node at both ingest and query); graph-store choice recorded in ADR-001 (stay relational, revisit at multi-hop scale); Testcontainers ITs. Phase 2 core complete. Phase 3 — Freshness & Governance 🔄 core complete: **checksum-driven re-index** already in the connector ingest path (`DefaultSourceIngestionService` — unchanged source is `UNCHANGED`/not re-embedded, changed re-indexed in place); **per-collection freshness policy** (`FreshnessPolicy` + `FreshnessPolicyStore`, V006 `collection_freshness_policy` — re-index interval + `autoReingest` opt-in; the freshness sweep honours each collection's interval via a correlated `COALESCE`, global default as fallback; `GET/PUT /api/v1/tenants/{tenantId}/collections/{collectionId}/freshness-policy`); and **GDPR collection erasure** (`KnowledgeErasurePort`/`DefaultKnowledgeErasureService` + `DELETE /api/v1/tenants/{tenantId}/collections/{collectionId}` — erases chunks → documents → graph entities with relations cascading, tenant+collection-scoped, idempotent, reports counts; `deleteByCollection` on the doc/chunk/graph stores); Testcontainers ITs. Phase 3 core complete: **auto re-ingestion of stale documents** now closes the last follow-up — `StaleReingestionPort`/`DefaultStaleReingestionService` reads the `autoReingest` scopes (`FreshnessPolicyStore.findAutoReingestScopes`), lists each scope's `STALE` documents (`KnowledgeDocumentStore.findByStatus`, tenant+collection-scoped) and re-fetches them through their source connector (`SourceIngestionPort`), driven by `StaleReingestionScheduler` (`aether.vault.freshness.reingest-cron`, default 04:30 after the freshness sweep; best-effort per document, a no-op when no connector is enabled; metered `aether.vault.reingest.{documents,failures}`). Phase 4 — Kubernetes + Helm 🔄 core complete: a production **Helm chart** (`vault-infra/helm/aether-vault/`) mirroring the ecosystem's hardened charts — namespace, service-account (token disabled), configmap, ClusterIP service (8084), deployment (non-root uid 1000, read-only rootfs, dropped caps, topology spread, probes, config-checksum rollout), **HPA** (min 2 / max 8 / CPU 70%), ingress, OpenShift Route, ServiceMonitor, NOTES; vanilla / AWS EKS (ALB + IRSA) / OpenShift (Route + SCC) value sets; secrets never in-chart (pre-existing `existingSecret`); and a **Helm lint+package release workflow** (`helm-release.yml`).
